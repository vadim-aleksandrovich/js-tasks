...
function timeSpanToString(startDate, endDate) {
  ...
  return `${addZero(hour)}:${addZero(min)}:${addZero(sec)}.${addZero(mSec)}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const hourArrow = 0.5 * (60 * hour + min);
  const minArrow = 6 * min;
  const ArrowAngle = hourArrow - minArrow > 180 ? hourArrow - minArrow - 180 : hourArrow - minArrow;
  return Math.abs((ArrowAngle * Math.PI) / 180);
}
